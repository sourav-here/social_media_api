...
import 'package:social_media_api/model/user_model.dart';
...
class UserService {
  static Future<List<UsersModel>> fetchAllUsers() async {
    Dio dio = Dio();
    final response =
        await dio.get('http://social-media-5ukj.onrender.com/user/');
  ...
  Future<void> followUser(BuildContext context, String id) async {
    ...
    try {
      final response = await dio.put(
          'http://social-media-5ukj.onrender.com/user/$loggedId/follow',
  ...
  Future<void> unFollowUser(BuildContext context, String id) async {
    ...
    try {
      final response = await dio.put(
          'http://social-media-5ukj.onrender.com/user/$loggedId/unfollow',
          data: jsonEncode(data),
          options: Options(
            headers: {
              'Content-Type': 'application/json',
